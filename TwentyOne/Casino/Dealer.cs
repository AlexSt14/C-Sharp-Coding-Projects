...
namespace Casino
{
    public class Dealer     //Dealer parent class
    {
        ...
        public void Deal(List<Card> Hand)       //Deal method that is called in play method of games, passing in as paramenter a list of cards Hand
        {
            ...
            using (StreamWriter file = new StreamWriter(@"D:\log.txt", true))     //using the "using" before, we make sure the memory gets cleaned up after we are done, creating a new streamwriter object, taking in the path where the text file is and takes another 
            {                                                                                   //argument "true" where it says "yes, I do want to append to the log file"
                file.WriteLine(DateTime.Now);       //Logging the date and time of each card that the dealer has given to the player
